...
import { Building, Lock, Mail, User, AlertCircle, ArrowRight, CheckCircle } from "lucide-react";
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        ...
        // If email confirmation is enabled, we notify the user. Otherwise, redirect to Dashboard.
        if (data.session) {
          router.replace("/");
        } else {
          setSuccessMsg(
            "Inscription réussie ! Veuillez vérifier vos e-mails pour confirmer votre inscription."
          );
        }
      }
    } catch (err) {
      ...
      <div className="w-full max-w-md space-y-8 bg-slate-900/40 backdrop-blur-xl border border-slate-800 p-8 rounded-3xl shadow-2xl relative z-10">
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-slate-400 font-medium">
            Créez votre compte bailleur en quelques instants
          </p>
        </div>
        ...
        {successMsg && (
          <div className="flex items-center gap-3 p-4 bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-sm rounded-2xl">
